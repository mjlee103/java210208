...
public class TomatoServer extends JFrame implements Runnable{
	//사용자가 접속을 해올 때 마다(입장) 새로 객체를 생성해야 함. 왜냐하면 서로 다른 사람들이기 때문, 그 사람의 소켓-ois-oos를 설계 해줘야 함. 
	TomatoServerThread 		tst 		= null;
	//Vector로 선언하지 않은 이유는 다형성을 위해서이다. 
	...
	//서버소켓과 클라이언트측 소켓을 연결하기
		...
		public void run() {
			...
			try {
				...
				while(!isStop) {
					socket = server.accept();//대기타는 중 
					jta_log.append("client info:"+socket+"\n");	//입장되면 client에 소켓 정보 뿌려줌. 			
					tst = new TomatoServerThread(this); //tomato server thread 생성 
					tst.start();//run 호출 
				}
			} catch (Exception e) {
				e.printStackTrace();
	...
	public static void main(String args[]) {
		...
		th.start(); //public void run과 연결되어야 함. 
