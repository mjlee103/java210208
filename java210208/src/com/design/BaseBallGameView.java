...
////////////메뉴바 추가하기 ///////////////
	...
	//컴파일 타임에 인스턴스화하기: 이른 인스턴스화이다. - 멀티 스레드에서 안전하지 못함, 메모리 과다 사용 문제 <- 해결방법: 싱글톤
	//파라미터를 갖는 생성자가 한개라도 있으면 어떠한 생성자도 제공하지 않음. 
	BaseBallGameLogic bbgLogic = new BaseBallGameLogic();
	BaseBallGameEvent bbgEvent = new BaseBallGameEvent(this, bbgLogic); //파라미터 자리에 2가지가 왔기 때문에 bblogic 생성자 추가해줘야 함. event의 #18 확인
	//BaseBallGameEvent bbgEvent = new BaseBallGameEvent(this); //파라미터 자리에 this 넣은것을 객체 주입이라고 한다. <- view와 event가 관련 있음을 보여줌. 
	//화면처리하기
...
//		jbtns[0].setBackground(newColor);
	...
	public static void main(String[] args) {
		BaseBallGameView bbgView = new BaseBallGameView();//생성자로 호출
		bbgView.initDisplay(); //직접 호출
	}
